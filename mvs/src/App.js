...
function App() {
  ...
  const recommendMovies = (searchedMovie) => {
    ...
    // Check the input movie and genre
    // console.log('Searched Movie:', searchedMovie);
    // console.log('gener',genre);
    // Filter the moviesData
    const results = moviesData.filter(movie => {
      const isSameGenre = movie.genre.toLowerCase() === genre.toLowerCase();
      const isDifferentTitle = movie.title.toLowerCase() !== searchedMovie.title.toLowerCase();
      console.log(`Movie: ${movie.title}, Genre: ${movie.genre}, isSameGenre: ${isSameGenre}, isDifferentTitle: ${isDifferentTitle}`);
      return isSameGenre && isDifferentTitle;
    });
