...
function App() {
  ...
  const searchMovies = (query) => {
    ...
    setSearchResults(results);
    if (results.length > 0) {
      recommendMovies(results[0]);
    } else {
      setRecommendations([]);
    }
  };

  const recommendMovies = (searchedMovie) => {
    const { genre } = searchedMovie;
    const results = moviesData.filter(movie =>
      movie.genre.toLowerCase() === genre.toLowerCase() && movie.title.toLowerCase() !== searchedMovie.title.toLowerCase()
    );
    setRecommendations(results);
  };
...
return (
  <div className="App">
    {!loggedIn ? (
      ...
      <>
        <SearchBar searchMovies={searchMovies} onLogout={handleLogout} />
...
// // Function to group movies by language
// const groupMoviesByLanguage = (movies) => {
//   return movies.reduce((acc, movie) => {
//     if (!acc[movie.language]) {
//       acc[movie.language] = [];
...
// // Function to filter movies by genre and language
// const filterMoviesByGenreAndLanguage = (movies, language, genre) => {
...
// function App() {
//   // State variables
//   const [movies, setMovies] = useState(moviesData); // Movies data state
//   const [searchResults, setSearchResults] = useState([]); // Search results state
//   const [recommendations, setRecommendations] = useState([]); // Recommended movies state
//   const [loggedIn, setLoggedIn] = useState(false); // Logged-in state
//   const [showSignUp, setShowSignUp] = useState(false); // Show sign-up form state
//   const [showRecommendations, setShowRecommendations] = useState(false); // Show recommendations state
...
//     setSearchResults(results);
//     if (results.length > 0) {
//       recommendMovies(results[0]); // Recommend movies based on first search result
//       setShowRecommendations(true); // Show recommendations when search results found
//     } else {
//       setRecommendations([]); // Reset recommendations if no search results
//     }
//   };

//   // Function to recommend movies based on searched movie
//   const recommendMovies = (searchedMovie) => {
//     const { language, genre } = searchedMovie;
//     const results = moviesData.filter(movie =>
//       movie.language.toLowerCase() === language.toLowerCase() &&
//       movie.genre.toLowerCase() === genre.toLowerCase() &&
//       movie.title.toLowerCase() !== searchedMovie.title.toLowerCase() // Exclude the searched movie from recommendations
//     );
//     setRecommendations(results);
//   };
...
//   const handleSignUp = (username, password, email) => {
//     console.log(`Signed up with username: ${username}, email: ${email}`);
//     setLoggedIn(true); // Set logged-in state to true after successful sign-up
//   };

//   // Function to handle user logout
//   const handleLogout = () => {
//     setLoggedIn(false); // Set logged-in state to false on logout
//     setShowSignUp(false); // Hide sign-up form on logout
//     setShowRecommendations(false); // Hide recommendations on logout
//   };

//   // Group movies by language for displaying in sections
//   const groupedMovies = groupMoviesByLanguage(searchResults.length ? searchResults : movies);

//   return (
//     <div className="App">
//       {!loggedIn ? (
//         // Show login form or sign-up form if not logged in
//         showSignUp ? (
//           <SignUp onSignUp={handleSignUp} />
//         ) : (
//           <LoginForm onLogin={handleLogin} onShowSignUp={() => setShowSignUp(true)} />
//         )
//       ) : (
//         // Show search bar, recommendations, or movie lists based on state
//         <>
//           <SearchBar searchMovies={searchMovies} onLogout={handleLogout} />
//           {showRecommendations && searchResults.length > 0 && <MovieRecommendations recommendations={recommendations} />}
//           {!showRecommendations && Object.keys(groupedMovies).map(language => (
//             <div key={language}>
//               <h2>{language} Movies</h2>
//               {Object.keys(groupedMovies[language]).map(genre => (
//                 <div key={genre}>
//                   <h3>{genre}</h3>
//                   <MovieList movies={filterMoviesByGenreAndLanguage(groupedMovies[language], language, genre)} />
//                 </div>
//               ))}
//             </div>
//           ))}
